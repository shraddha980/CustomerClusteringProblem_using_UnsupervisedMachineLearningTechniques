from Segmentation.entity import artifact_entity,config_entity
from Segmentation.exception import SegmentationException
...
import numpy as np
from Segmentation.config import TARGET_COLUMN
...
class DataValidation:
    ...
    def initiate_data_validation(self)->artifact_entity.DataValidationArtifact:
        try:
            ...
            logging.info(f"Converting categorical Columns to Numerical")
            base_df_con = utils.convert_columns_cat_to_num(df=base_df)
            logging.info(f"Converting base columns to encoded columns: {base_df_con.head(10)}")
            logging.info(f"Converting base columns to encoded columns: {base_df_con.shape}")
            train_df_con = utils.convert_columns_cat_to_num(df=train_df)
            logging.info(f"Converting train columns to encoded columns: {train_df_con.head(10)}")
            logging.info(f"Converting base columns to encoded columns: {base_df_con.shape}")
            test_df_con = utils.convert_columns_cat_to_num(df=test_df)
            logging.info(f"Converting test columns to encoded columns: {test_df_con.head(10)}")
            logging.info(f"Converting base columns to encoded columns: {base_df_con.shape}")

            exclude_columns = [TARGET_COLUMN]
            base_df = utils.convert_columns_float(df=base_df_con, exclude_columns=exclude_columns)
            train_df = utils.convert_columns_float(df=train_df_con, exclude_columns=exclude_columns)
            test_df = utils.convert_columns_float(df=test_df_con, exclude_columns=exclude_columns)
