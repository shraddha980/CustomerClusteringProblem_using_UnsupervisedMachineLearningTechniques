...
class ModelTrainer:
    ...
    def initiate_model_trainer(self,)->artifact_entity.ModelTrainerArtifact:
        try:
            ...
            logging.info(f"x_train,y_train: {x_train.shape},{y_train.shape}")
            x_test,y_test = test_arr[:,:-1],test_arr[:,-1]
            logging.info(f"x_test,y_test:  {x_test.shape},{y_test.shape}")
            ...
            logging.info(f"yhat_train: {yhat_train}")
            #r2_train_score  =r2_score(y_true=y_train, y_pred=yhat_train)

            logging.info(f"Calculating f1 test score")
            yhat_test = model.predict(x_test)
            logging.info(f"yhat_test : {yhat_test}")
