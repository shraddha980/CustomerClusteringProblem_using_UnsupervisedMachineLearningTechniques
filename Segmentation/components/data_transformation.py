from Segmentation.entity import config_entity, artifact_entity
...
class DataTransformation:
    ...
    def initiate_data_transformation(self,) ->artifact_entity.DataTransformationArtifact:
        try:
            ...
            logging.info(f" Train_df: {train_df.head(10)}")
            test_df = pd.read_csv(self.data_ingestion_artifact.test_file_path)
            logging.info(f"Test_df: {test_df.head(10)} ")

            input_feature_train_df = train_df.drop(TARGET_COLUMN, axis =1)
            logging.info(f" Input_Feature_Train_df : {input_feature_train_df.head(10)}")
            input_feature_test_df = test_df.drop(TARGET_COLUMN, axis =1)
            logging.info(f" Input_Feature_test_df :  {input_feature_test_df.head(10)}")

            target_feature_train_df = train_df[TARGET_COLUMN]
            logging.info(f" Target_feature_train_df: {target_feature_train_df.shape}")
            target_feature_test_df = test_df[TARGET_COLUMN]
            logging.info(f" Target_feature_test_df :  {target_feature_test_df.shape}")

            label_encoder = LabelEncoder()
            ## fit Data
            target_feature_train_arr = target_feature_train_df.squeeze()
            logging.info(f"target_feature_train_arr : {target_feature_train_arr.shape}")
            target_feature_test_arr = target_feature_test_df.squeeze()
            logging.info(f"target_feature_test_arr: {target_feature_test_arr.shape}")
            ...
            logging.info(f"input_feature_test_arr: {input_feature_test_arr.shape}")

            train_arr = np.c_[input_feature_train_arr, target_feature_train_arr]
            logging.info(f"train_arr: {train_arr.shape}")
            test_arr = np.c_[input_feature_test_arr, target_feature_test_arr]
            logging.info(f"test_arr: {test_arr.shape}")
            ...
            logging.info(f"Data_Transformation_Artifact: {data_transformation_artifacts}")
